...
else
{
	...
	if($arParams["SHOW_MENU"] == "Y")
	{
        ...
        <ul class="card-nav-list" style="display: inline-block;">
			<?if ($arResult["CurrentStep"] < 6):?>
				...
				for ($i = 0; $i < count($arMenuLine); $i++)
				{
					...
					if ($arResult["CurrentStep"] > $i + 1)
						echo "<li class='card-nav-list-item'><a class='card-nav-list-item__link' href=\"#\" OnClick=\"document.order_form.CurrentStep.value='".($i + 1)."'; document.order_form.BACK.value='Y'; document.order_form.submit();\">".$arMenuLine[$i]."</a></li>";
					elseif ($arResult["CurrentStep"] == $i + 1)
						echo "<li class='card-nav-list-item'><a class='card-nav-list-item__link active' href=\"#\" OnClick=\"document.order_form.CurrentStep.value='".($i + 1)."'; document.order_form.BACK.value='Y'; document.order_form.submit();\">".$arMenuLine[$i]."</a></li>";
					else
						echo "<li class='card-nav-list-item'>".$arMenuLine[$i]."</li>";
				}
				?>
